...
namespace Sphere {
  ...
  bool Point::operator==(const Point& other) const {
    return (std::abs(latitude - other.latitude) < std::numeric_limits<double>::epsilon() ) && (std::abs(longitude - other.longitude) < std::numeric_limits<double>::epsilon());
  }

  std::ostream& operator<<(std::ostream& os, const Sphere::Point& point) {
    os << "{\n";
    os << GuardedString("lat") << ": " << "\"" << point.latitude << "\"" << ",\n";
    os << GuardedString("lon") << ": " << "\"" << point.longitude << "\"" << "\n";
    os << "}";
    return os;
  }
}
