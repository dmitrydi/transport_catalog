...
string TransportCatalog::Serialize() const {
  ...
  database.Serialize(*db_proto.mutable_yp_db());
...
std::vector<std::string> TransportCatalog::FindCompanies(const Filters::Filter& filter) const {
  ...
  for (const auto& company: database.companies) {
    if (CompanyMatch(company, filter.names, filter.urls, filter.phones, filter.rubrics, mapToRubric)) {
      result.push_back(company.GetMainName());
    }
  }
  return result;
}
