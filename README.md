...
где "filter" - словарь фильтров на организации. Фильтры бывают следующих типов:  
1. `"rubrics": ["rubric1", ....]` - фильтр на категорию компании. Значения фильтров должны содержаться в значения словаря "rubrics" раздела "yellow_pages" при запуске опции "make_base".  
2. `"urls": ["url1", ...]` - фильтр на url организации. Ищется точное совпадение среди значений фильтра и значений url компании.  
3. `"names": ["name1", ...]` - фильтр на название компании. Ищется точное совпадение среди значений фильтра и названий компании.  
4. `"phones" : [PhoneFilter1, ...]` - фильтры на номера телефонов. Каждый фильтр может содержать следующие ключи:  
  * type - строка "PHONE" или "FAX"  
  * country_code — строка  
  * local_code — строка  
  * number — строка  
  * extension — строка  
  при этом пустые строки эквивалентны отсутсвию соотв. ключа.
  Самый простой шаблон, соответствующий номеру телефона, содержит те же type, country_code, local_code, number и extension. Помимо этого, номер телефона с добавочной частью (extension) или типом (type) должен находиться по шаблону без добавочной части или типа. Наконец, в шаблоне может быть не указан код страны (country_code) или оба кода — страны (country_code) и города (local_code). Соответственно, при отсутствии в шаблоне местного номера (number) или при отсутствии local_code и одновременно наличии country_code не найдётся ни один номер, содержащий непустые number, country_code и local_code.  

Например, по шаблону {"type": "PHONE", "number": "3633659"} могут найтись следующие номера телефонов:  
...
При наличии нескольких фильтров (`name, url, ...`) компания должна удовлетворять каждому из них.  
Отсутсвующий или пустой фильтр означает отсутсвие ограничений по данному признаку.  
...
где `RouteItem1, ...` - элементы маршрута до компании. В добавление к типам "Wait" и "Bus", аналогичным запросу `Route`, существует тип "Walk", обозначающий отрезок маршрута от поседней остановки до организации: `{"stop_from": "last_stop_name", "time": double, "to_company": "company_name", "type": "Walk"}`.

`total_time` - общее время следования по маршруту с учетом ожидания на остановках.  

`map` - svg-документ - схема автобусов с построенным маршрутом. Пример маршрута на автобусной схеме:

![rtc1](./test_out/rtc_response.jpg)

[Пример](./test_inputs/sample_requests.txt) запросов к сериализованному каталогу.
[Вывод программы](./test_out/sample_output.txt) в ответ на запросы.

### Запуск внутренних тестов (опция run_tests)

Опция запускает встроенные тесты. После выполнения тестов программа завершается.  
